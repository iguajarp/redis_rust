...
impl RespConnection {
    ...
    pub async fn read_value(&mut self) -> Result<Option<Value>> {
        loop {
            ...
            // parse_message needs to return a second value for other purpuses in the code.
            // ie: if the message is an array (*n), it will return the length of the array and the bytes consumed to read that.
...
fn parse_message(buffer: BytesMut) -> Result<Option<(Value, usize)>> {
    match buffer[0] as char {
        ...
        _ => Err(Error::msg("unrecognised msg type")),
    }
...
/// Get the length of the array and its len.
/// Use the length and get every command from the array-string
fn decode_array(buffer: BytesMut) -> Result<Option<(Value, usize)>> {
    let (array_length, mut bytes_consumed) = 
        if let Some((line, len)) = read_until_crlf(&buffer[1..]) {
            ...
            (array_length, len + 1) // +1 because it need to start from the next character after CRLF
        } else {
            return Ok(None);
        };
...
/// Return buffer from first character unitl before carriage_return. Returns the length of the string too.
fn read_until_crlf(buffer: &[u8]) -> Option<(&[u8], usize)> {
    // start at 1 to check if current and i - 1 is the end of the line with crlf
